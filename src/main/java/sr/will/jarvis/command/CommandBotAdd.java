...
public class CommandBotAdd extends Command {
    ...
    public void execute(Message message, String... args) {
        ...
        if (jarvis.chatterBotManager.isBotChannel(message.getChannel().getId())) {
            message.getChannel().sendMessage(new EmbedBuilder().setTitle("Error").setColor(Color.RED).setDescription("Bot already active in this channel").build()).queue();
            return;
        }

        message.getChannel().sendMessage(new EmbedBuilder().setTitle("Success").setColor(Color.GREEN).setDescription("Bot added to channel").build()).queue();
