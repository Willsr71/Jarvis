...
public class StatsdClient extends TimerTask {
    ...
    private synchronized boolean doSend(String stat) {
        try {
            ...
            log.severe(String.format("Could not send stat %s to host %s:%d", sendBuffer.toString(), _address.getHostName(), _address.getPort()));
            e.printStackTrace();
            return false;
        }
    }
    ...
    public synchronized boolean flush() {
        try {
            ...
            if (sizeOfBuffer == nbSentBytes) {
                ...
                log.severe(String.format("Could not send entirely stat %s to host %s:%d. Only sent %d bytes out of %d bytes", sendBuffer.toString(), _address.getHostName(), _address.getPort(), nbSentBytes, sizeOfBuffer));
                return false;
            }
